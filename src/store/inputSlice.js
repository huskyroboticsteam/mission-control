...
function computeDriveInput(state, action) {
  ...
  driveInput.straight =
    -driveGamepad["LeftStickY"] +
    getAxisFromKeys(pressedKeys, "ARROWDOWN", "ARROWUP");
  driveInput.steer =
    driveGamepad["RightStickX"] +
    getAxisFromKeys(pressedKeys, "ARROWLEFT", "ARROWRIGHT");
  driveInput.left =
    driveGamepad["LeftStickY"] +
    getAxisFromKeys(pressedKeys, "ARROWDOWN", "ARROWLEFT");
  driveInput.right =
    driveGamepad["RightStickY"] +
    getAxisFromKeys(pressedKeys, "ARROWRIGHT", "ARROWUP");
  driveInput.crab =
    driveGamepad["LeftStickX"] +
    getAxisFromKeys(pressedKeys, "ARROWDOWN", "ARROWUP");
...
function computeArmInput(state) {
  ...
  armInput.armBase =
    peripheralGamepad["LeftStickX"] + getAxisFromKeys(pressedKeys, "A", "D");
  if (state.inverseKinematics.enabled) {
    armInput.ikForward =
      -peripheralGamepad["LeftStickY"] + getAxisFromKeys(pressedKeys, "S", "W");
    armInput.ikUp =
  ...
  armInput.forearm =
    peripheralGamepad["RightStickX"] + getAxisFromKeys(pressedKeys, "F", "H");
  armInput.wristPitch =
...
function computeScienceInput(prevState, state, action) {
  ...
  scienceInput.drillMotor =
    toggleKey(prevPressedKeys, pressedKeys, "B", scienceInput.drillMotor) +
    toggleButton(prevPeripheralGamepad, peripheralGamepad, "LB", scienceInput.drillMotor);
}
...
function toggleButton(prevPeripheralGamepad, peripheralGamepad, button, currState) {
  if (!prevPeripheralGamepad[button] && peripheralGamepad[button]) {
    return !currState;
  }
  return currState;
}
