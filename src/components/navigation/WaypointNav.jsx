  ...
  return (
    <form method="post" onSubmit={handleSubmit} className="waypoint-select">
      <div className="waypoint-select__params">
        ...
        <input disabled={!roverIsConnected || isWaypointSet} type="number" step="any" name="latitude" value={lat} onChange={e => setLat(e.target.value)} />
        <label htmlFor="longitude">Longitude</label>
        <input disabled={!roverIsConnected || isWaypointSet} type="number" step="any" name="longitude" value={lon} onChange={e => setLon(e.target.value)} />
        <button disabled={!roverIsConnected || isWaypointSet} type="button" onClick={grabFromClipboard}>Copy from Clipboard</button>
      </div>
      <div className="waypoint-checkbox">
        <label><input disabled={!roverIsConnected || isWaypointSet} type="checkbox" name="isApproximate" /> Approximate</label>
        <label><input disabled={!roverIsConnected || isWaypointSet} type="checkbox" name="isGate" /> Is Gate</label>
      </div>
      {
        isWaypointSet ?
          ...
          <button disabled={!roverIsConnected} type="button" onClick={handleWaypoint}>Set Waypoint</button>
      }
